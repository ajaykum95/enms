...
public class RequestValidator {
    ...
    public static void validateLeadStatusRequest(
            RequestEntity<LeadStatusRequest> leadStatusRequestEntity) {
        if (Objects.isNull(leadStatusRequestEntity) || !leadStatusRequestEntity.hasBody()) {
            throw buildException(EnmsExceptions.LEAD_STATUS_REQUEST_MISSING);
        }
        if (StringUtils.isEmpty(leadStatusRequestEntity.getBody().getLeadStatus())) {
            throw buildException(EnmsExceptions.LEAD_STATUS_MISSING);
        }
    }
}
